...
const tempText = useTypingEffect(
  'Ensure that the server room temperature remains below 30°C at all times.',
)
const humText = useTypingEffect('Maintain humidity levels below 80% for optimal conditions.')
const fireText = useTypingEffect(
  'This system detects fire using a sensor and displays the fire status in real-time. If fire is detected, a visual alert will be triggered.',
)
...
const fetchData = async () => {
  try {
    ...
    sensorData.value = jsonData.slice(-10)
...
<template>
  <section id="current" class="flex gap-4">
    ...
    >
      ...
      <div
        class="text-center text-xs text-neutral-400 min-h-[40px] flex items-center justify-center"
      >
        <span>{{ tempText }}</span>
      </div>
    </div>
    ...
    >
      ...
      <svg
        ...
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        class="absolute scale-50 xl:scale-100 left-8 md:left-16 lg:left-12 xl:left-20 top-10 md:top-20 md:opacity-0 md:transition-all md:duration-700 md:ease-out"
        :class="{ 'md:opacity-100 md:top-[48px]': isVisible }"
      >
        >
        <path
    ...
    >
      <svg
        ...
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        class="absolute scale-75 xl:scale-100 md:left-16 lg:left-10 xl:left-[70px] top-0 md:top-20 opacity-0 transition-all duration-700 ease-out"
        :class="{ 'opacity-100 top-8 md:top-[52px]': isVisible }"
      >
        <path
      ...
      <div
        class="text-center text-xs text-neutral-400 min-h-[50px] flex items-center justify-center"
      >
        <span>{{ fireText }}</span>
      </div>
    </div>
